...
function lcso_init_fn() {
	...
	// if special keys are used or optimization is disabled - stop
		...
		// if optimization is on testing mode and current user isn't admin - stop
...
class lcso_init {
	...
	function __construct() {
		...
		if(isset($_REQUEST['lcso_store_scripts'])) {
			add_action('wp_print_scripts', array($this, 'store_js_queued_elements'), 99999);	
			add_action('wp_print_styles', array($this, 'store_css_queued_elements'), 99999);
			add_action('wp_head', array($this, 'shutdown'), 100);
		...
		add_action('wp_head', array($this, 'print_head'), 15);
		add_action('wp_footer', array($this, 'print_footer'), 15);	
	}
...
';
	...
	public function store_js_queued_elements() {
		...
		foreach($wp_scripts->queue as $s) {
			if(isset($wp_scripts->registered[$s])) {	
				$queued_js[$s] = $wp_scripts->registered[$s];
			}
		}
	...
	public function store_css_queued_elements() {
		...
		foreach($wp_styles->queue as $s) {
			if(isset($wp_styles->registered[$s])) {
				$queued_css[$s] = $wp_styles->registered[$s];
			}
		}
	...
	public function head_foot_skim_js() {
		...
		foreach($queued as $s) {
			if(!isset($regist[$s]) || in_array($s, $this->js_to_ignore)) {continue;}
	...
	/* manage script inclusion - standalone function to be recursive
	 *
	...
	private function script_inclusion($script_id, $caller_key = '') {
		...
		if(!isset($regist[$s])) {return false;}
		
		// check dependencies
		if(!empty($regist[$s]->deps)) {
			foreach($regist[$s]->deps as $dep) {
				if(!isset($regist[$dep]) || in_array($dep, $this->js_to_ignore)) {continue;}	
	...
	public function head_foot_skim_css() {
		...
		foreach($queued as $s) {
			if(!isset($regist[$s]) || in_array($s, $this->css_to_ignore)) {continue;}
	...
	/* manage style inclusion - standalone function to be recursive
	 *
